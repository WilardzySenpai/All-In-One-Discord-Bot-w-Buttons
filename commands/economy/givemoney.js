const { MessageEmbed } = require("discord.js");
const discord = require("discord.js");
const emote = require("../../config/emotes.json");
require('discord-reply');
const client = new discord.Client();
const db = require("quick.db");

module.exports = {
    name: "givemoney",
    aliases: ["give", "pay"],
    category: "economy",
    description: "Give Money To Somebody",
    usage: "givemoney [mention | ID] <amount>",
    
  run: async (bot, message, args) => {
    
try {
  let user2 = message.author
    if (!args[0]) return message.channel.send("**Mention A User!**");
    let user =
      message.mentions.members.first() ||
      message.guild.members.cache.get(args[0]) ||
      message.guild.members.cache.find(
        r => r.user.username.toLowerCase() === args[0].toLocaleLowerCase()
      ) ||
      message.guild.members.cache.find(
        r => r.displayName.toLowerCase() === args[0].toLocaleLowerCase()
      );
    if (!user) return message.channel.send("**Mention A Valid User!**");

    let member = db.fetch(`money_${user2.id}`);

    let embed1 = new MessageEmbed()
      .setColor("RED")
      .setDescription(`Mention Someone To Give Money`);

    if (!args[0]) {
      return message.channel.send(embed1);
    }
    let embed2 = new MessageEmbed()
      .setColor("RED")
      .setDescription(`You Cannot Give Money To Yourself`);

    if (user.user.id === message.author.id) {
      return message.channel.send(embed2);
    }

    let embed3 = new MessageEmbed()
      .setColor("RED")
      .setDescription(`Specify An Amount To Pay`);

    if (!args[1]) {
      return message.channel.send(embed3);
    }
    let embed4 = new MessageEmbed()
      .setColor("RED")
      .setDescription(`Enter A Valid Amount!`);

    if (isNaN(args[1])) {
      return message.channel.send(embed4);
    }
    let embed5 = new MessageEmbed()
      .setColor("RED")
      .setDescription(`You Don't Have That Much Money`);

    if (member < args[1]) {
      return message.channel.send(embed5);
    }

    let embed6 = new MessageEmbed()
      .setColor("GREEN")
      .setDescription(`You Gave ${args[1]}${emote.coin} To **${user.displayName}**`);

    message.lineReplyNoMention(embed6);
    db.add(`money_${user.id}`, args[1]);
    db.subtract(`money_${user2.id}`, args[1]);
    } catch {
        
    }
  }
};